...
exports.verify = async(req, res) => {
    ...
    try {
        ...
        User.findOne({ _id: token.userId}, (err, user) => {
            if (!user) return res.status(400).json({ messasge: 'We were unable to find a user for this token.'});
...
exports.resendToken = async(req, res) => {
    try {
        ...
        if (!user) return res.status(401).json({ message: 'The email address ' + req.body.email + ' is not associcated with any account. Double-check your email address and try again.'});
...
async function sendVerificationEmail(user, req, res) {
    try {
        ...
        let html = `<p>Hi ${user.firstName} ${user.lastName}<p><br><p>Please click on the following <a href="${link}">link</a> to verify your account.
